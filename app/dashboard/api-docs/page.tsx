...
export default function ApiDocsPage() {
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/dashboard/settings" className="text-sm text-gray-400 hover:text-gray-600">← Paramètres</Link>
      </div>

      <h2 className="text-2xl font-bold text-gray-900 mb-1">Documentation API</h2>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        ...
        <p className="text-sm text-gray-500 mb-3">
          Toutes les requêtes nécessitent un header <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">Authorization</code> avec votre clé API.
        </p>
  ...
  -H "Authorization: Bearer sf_live_votrecle..."`}</pre>
        <p className="text-xs text-gray-400 mt-3">
          ...
          <Link href="/dashboard/settings" className="text-blue-600 hover:underline">Paramètres → Clés API</Link>.
        </p>
      </div>
        ...
        {ENDPOINTS.map((ep, i) => (
          <div key={i} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center gap-3">
              ...
              <span className="text-sm text-gray-400 ml-auto">{ep.title}</span>
            </div>
            <div className="px-5 py-4 space-y-3">
              ...
              {ep.body && (
                <div>
                  <p className="text-xs font-medium text-gray-400 uppercase mb-1">Body (JSON)</p>
                  <pre className="bg-gray-900 text-blue-300 rounded-lg p-3 text-xs overflow-x-auto">{ep.body}</pre>
                </div>
              )}

              <div>
                <p className="text-xs font-medium text-gray-400 uppercase mb-1">Réponse</p>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 mt-6">
        <h3 className="font-semibold text-gray-900 mb-3">Codes de réponse</h3>
        <table className="w-full text-sm">
          <tbody className="divide-y divide-gray-50">
            {[
              ...
              <tr key={code}>
                <td className="py-2 pr-4 font-mono font-semibold text-gray-900 w-12">{code}</td>
                <td className="py-2 pr-4 text-gray-600 w-40">{status}</td>
