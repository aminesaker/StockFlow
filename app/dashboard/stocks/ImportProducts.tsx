  ...
  if (!open) {
    return (
      <button
        ...
        className="px-3 py-2 border border-border text-muted-foreground text-sm rounded-lg hover:bg-muted/40 transition-colors"
      >
        ⬆ Importer CSV
      </button>
    )
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="rounded-xl bg-card shadow-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold text-foreground">Importer des produits</h2>
        ...
        <p className="text-sm text-muted-foreground mb-4">
          ...
          <code className="bg-muted px-1 rounded text-xs">name, sku, price, stock_quantity</code> (obligatoires).
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition-colors ${
            dragOver ? 'border-blue-400 bg-blue-50' : 'border-border hover:border-input hover:bg-muted/40'
          }`}
        >
          <input
            ref={inputRef}
          ...
          {selectedFile ? (
            ...
            <div>
              <p className="text-sm text-muted-foreground">Glissez un fichier CSV ici</p>
